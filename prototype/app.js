...
function buildPosts(data) {
  ...
  return Array.from({ length: 7 }, (_, index) => {
    ...
    return {
      title: `D${index + 1}｜${rhythm}：${motive}的${data.storeType}选择`,
      body: `${data.storeName} 主推 ${data.product}，客单价 ${data.price}，面向${data.audience}。内容强调“${rhythm} + ${motive}”，结尾绑定动作：${profile.conversion}。`
    };
  });
}
  ...
  return [
    {
      ...
      body: `镜头 1 拍${profile.rhythm[0]}的人流；镜头 2 展示 ${data.product}；镜头 3 强调 ${profile.motivation[0]} 和 ${data.price}；结尾引导 ${profile.conversion}。`
    },
    {
      title: "真实购买理由采访",
      body: `找 2-3 位目标顾客说购买理由，围绕“${profile.motivation.slice(0, 3).join("、")}”，避免广告腔。`
    },
    {
      title: "老板 30 秒承诺",
      body: `老板出镜说明本周目标“${data.goal}”，承诺一个具体改进：更快、更稳、更清楚优惠或更好售后。`
    }
  ];
}
...
function buildCommunity(data) {
  ...
  return {
    messages: [
      ...
      `${profile.label}专属提醒：赶时间可以提前发消息，尽量帮你快取。`,
      `适合${profile.rhythm[4]}，可以和熟人一起拼单，价格更划算。`,
      `本周验证一个小活动：${profile.conversion}，欢迎试一次。`,
      `老顾客回访：这周主推 ${data.product}，特殊需求可以提前说。`,
...
function buildValidation(data) {
  const profile = profileOf(data);
  return [
    ["D1-D2", `访谈 8-10 家${locationProfiles[data.locationContext]}商家，确认是否服务${profile.label}。`],
    ["D3-D4", "选择 3 家试用店，采集门头、菜单、评论区和高峰时段。"],
    ["D5-D6", `生成体检分、内容日历、短视频脚本和${profile.label}转化话术。`],
    ["D7-D9", `让商家在${profile.channel}发布，记录采用率。`],
    ["D10-D11", `围绕${profile.metric}收集数据。`],
    ["D12-D13", "优化客群画像、Prompt、定价和交付格式。"],
    ["D14", "测试 99/299 元付费意愿。"]
  ];
}
...
function updateChrome(data) {
  ...
  profileTitle.textContent = `${profile.label} · ${profile.rhythm.slice(0, 3).join("/")}`;
  strategyText.textContent = `围绕“${profile.rhythm.slice(0, 3).join("、")}”的节奏，把 ${data.product} 变成可追踪动作：${profile.conversion}。`;
  experimentTitle.textContent = `${profile.rhythm[0]}增长实验`;
  experimentCopy.textContent = `用“${profile.motivation[0]}”作为卖点，追踪${profile.metric}。`;

  healthScore.textContent = scores.health;
  scoreRing.style.setProperty("--score", scores.health);
  document.querySelector("#sceneScore").textContent = scores.scene;
  document.querySelector("#sceneLabel").textContent = profile.sceneLabel;
  ...
  evidenceList.innerHTML = [
    `${profile.label}有明确节奏：${profile.rhythm.slice(0, 3).join("、")}。`,
    `验证重点不是文案好看，而是${profile.metric}是否提升。`,
    `访谈要问是否已有${profile.conversion}。`,
    "关键付费问题：99 元试用包或 299 元月包能否接受。"
...
function renderDiagnosis(data) {
  ...
  output.innerHTML = `
    <article class="output-card wide">
      <h3>诊断结论</h3>
      <p>${data.storeName} 当前最值得验证的是“${data.goal}”。系统识别主要客群为“${profile.label}”，增长重点是把“${profile.motivation.slice(0, 3).join("、")}”转成可记录动作，而不是单纯多发内容。</p>
    </article>
    <article class="output-card">
      <h3>优势</h3>
      <ul>
        <li>客群画像清晰：${data.audience}</li>
        <li>购买动机明确：${profile.motivation.join("、")}</li>
    ...
    <article class="output-card wide">
      ...
      <div class="message-grid">
        <div class="message-item">实验 A：围绕“${profile.rhythm[0]}”发布，指标：${profile.metric}。</div>
...
function renderValidation(data) {
  ...
  output.innerHTML = `
    <article class="output-card wide">
      <h3>两周验证计划</h3>
      <div class="message-grid">
        ${timeline.map(([day, task]) => `<div class="message-item"><strong>${day}</strong><br>${task}</div>`).join("")}
      </div>
    </article>
  `;
...
if (!applyPreset(initialPreset, true)) render();
